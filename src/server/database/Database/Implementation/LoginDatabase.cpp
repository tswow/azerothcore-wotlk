...
// @tswow-begin - use trinitycore auth column names
//   account_access.gmlevel -> account_access.SecurityLevel
//   account_access.id -> account_access.AccountID
//   session_key -> session_key_auth (todo: is this safe?)
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_RECONNECTCHALLENGE,
        ...
        "LEFT JOIN account_banned ab ON ab.id = a.id AND ab.active = 1 "
        "LEFT JOIN ip_banned ipb ON ipb.ip = ? "
        "WHERE a.username = ? AND a.session_key_auth IS NOT NULL", CONNECTION_ASYNC);
